...
#Number of childern
if 1+1 < 3:
    children_sum = int(input('Input number of children, that are not 18 years old: '))
...
#Children number less than 2
    if children_sum <= 1:
        print('You wont get any money if you have less then 2 children below 18 years old')
        more_children = str(input('Do you have more then 2 children? (Yes/No)'))
    ...
    print('You gonna get ~', (18 * (i + 1) - (a * (i + 1) - sum(years_sum))) * 12 * 500 - (
                e * (i + 1) * 500 + sum(month_sum) * (i + 1) * 500), ' zł')
    input()
